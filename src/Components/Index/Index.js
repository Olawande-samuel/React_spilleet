...
import { SearchContext, UserContext } from "../Auth/User";
...
const Index = () => {
  ...
  const changeCategory = (e) => {
    ...
    Fetch("https://spilleetapi.spilleet.com/display-content", formData)
      .then((res) => {
        setLoading(false);
  ...
  return (
    ...
    <div>
      {/* <Head>
        <title>Spilleet | Home</title>
        <meta name="description" content={` ${username} Spilleet page`} />
        <meta name="keywords" content={` ${username}, Spilleet page`} />
        ...
        <main className={styles.main}>
          ...
          <div className={styles.mainContainer}>
            ...
            {loading ? (
              ...
              <>
                {data.length > 0 &&
                  data.filter((item)=> item.body.toLowerCase().includes(search) || item.title.toLowerCase().includes(search) || item.username.toLowerCase().includes(search)).map((item) =>
                    item.image_url === 0 ? (
                      <TextPost
                        key={item.ctn_id}
                        item={item}
