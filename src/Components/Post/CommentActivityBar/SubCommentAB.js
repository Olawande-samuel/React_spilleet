...
const SubCommentAB = ({upvotes,reply, item, handleClick, reloadComments}) => {
  ...
  if (typeof window !== "undefined") {
    ...
    return (
        <Grid container xs={12} justifyContent="space-between" alignItems="center">
            <Grid item xs={6} >
            <Box sx={{ display: "flex", alignItems: "center", gap: ".5rem" }} className={Styles.buttonGroup} >
            <Box sx={{ display: "flex", alignItems: "center" }}>
                <ArrowDropUpIcon sx={{ fontSize: "30px", color: "#30C06A" }}  onClick={handleLike} />
                {/* <span>Upvote</span> */}
                <span className={Styles.upvoteValue}>{`${ item.total_likes !== undefined ? totalLikes : 0 }`}</span>
            </Box>
          <Box
            sx={{
              display: "flex",
